...
public class Main {
	...
	public static List<String> allWhoseZipContains3() {
		...
		mems.addAll(members);
		//implement
		return mems.stream().map((e) -> e.getMemberId()).collect(Collectors.toList());
	...
	//Returns a list of all isbns of books having at least two copies
		public static List<String> allHavingAtLeastTwoCopies() {
			...
			bs.addAll(books);
			//implement
			return bs.stream().map((e) -> e.getIsbn()).collect(Collectors.toList());
	...
	public static List<String> allHavingMultipleAuthors() {
		...
		bs.addAll(books);
		//implement
		return bs.stream().filter((e) -> e.getAuthors().size()>0 ).map((e) -> e.getIsbn()).collect(Collectors.toList());
