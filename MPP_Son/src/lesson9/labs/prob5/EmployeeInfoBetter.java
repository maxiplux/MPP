...
public class EmployeeInfoBetter {
	...
	HashMap<SortMethod, Comparator> sortAlgorithms;
	
	EmployeeInfoBetter()
	{
		sortAlgorithms = new HashMap<>();
		sortAlgorithms.put(SortMethod.BYNAME, Comparator.comparing(byName).thenComparing(bySalary));
		sortAlgorithms.put(SortMethod.BYSALARY, Comparator.comparing(bySalary).thenComparing(byName));
	}
	
	public void sort(List<Employee> emps, final SortMethod method) {
		emps.sort(sortAlgorithms.get(method));	
	}
