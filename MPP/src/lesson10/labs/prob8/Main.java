...
public class Main {
	...
	private boolean findProduct(String prodName) {
		...
		// this.orderItems.stream().map((x) -> Optional.of(x)).flatMap((x) ->
		// x.getProduct)

		// this.orderItems.stream().filter(i -> i != null).filter(item ->
		// item.getProduct().flatMap((x) -> x.getProductName()
		// ).orElse("").equals(prodName)).findAny().isPresent();

		return this.orderItems.stream().anyMatch(item -> item != null
				&& item.getProduct().flatMap((x) -> x.getProductName()).orElse("").equals(prodName));
	}
	...
	// if(item != null)
	// {
	// if (item.getProduct().flatMap((x) -> x.getProductName()
