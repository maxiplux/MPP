...
public class EmployeeInfoBetter {
	...
	{
		HashMap<SortMethod, Comparator<Employee> > mapcomparator = new HashMap<SortMethod, Comparator<Employee>>();		
		mapcomparator.put(SortMethod.BYNAME, Comparator.comparing(byName).thenComparing(bySalary));		
		mapcomparator.put(SortMethod.BYSALARY,  Comparator.comparing(bySalary).thenComparing(byName));		
		Collections.sort(emps, mapcomparator.get(method));			
	}
